...
{
    // --------------значения и ключи для сесиий----------------- 
    private static $SCAN_ON = 1; //  нужно скaнить
    ...
    // функция для проверки статуса скана и отправкие его на клиента
    // в случае если скан завершился и требуемые данные есть в бд
    //
    // сделать запросы на бд, по тем типам скана, которые
    // помечены в сесии как SCAN_ON для требуемого ulr
    // получить данные из бд, отправить в шаблонизатор 
    // вернуть страницу на клиента
    // если данных нет - редирект на ту же самую страницу
    // откуда пришел запроc
    public function get_results(){
        ...
        //функция для запука сканирования
        //
        // принимает список задач и цель для сканирования
        // в сесиию записываются все сканирования, которые будут проводиться
        // данные для скана идут на python сервис, который просканит сайт и 
        // запишет данные в базу данных
        public function scan(Request $req){
            ...
            // для каждого нового запроса на скан почистить сессию
            // установить в сесиию сканируемый url и типы сканирования
            // которые будут проводиться над этим url
            session()->flush();
            foreach ($keys as $key){
                if ($key === 'url'){
                    $full_url = $data[$key];
            ...
            // если скан определенной техникой не требуется, пометить это в сесии
            ...
            // формируется запрос на python сервис
            ...
            // сделать запрос с python сервису с просьбой начать сканирование адреса способами, помеченными как on
            // и сделать редирект на страницу, которая будет отправлять запросы на получение результатов скана
